...
namespace JukeBoxSolutions.Controls
{
    ...
    /// </summary>
    public partial class LibraryCardSong : System.Windows.Controls.UserControl
    {
        ...
        public LibraryCardSong(AlbumTrack track, string trackNumber = "", LibraryPopup parentAlbum = null, bool isSelected = false, bool isInPlaylist = false)
        {
            InitializeComponent();
        ...
        public LibraryCardSong(TrackLibrary trackLibrary, bool isPlaying, int playlistSequence)
        {
            ...
            try
            {
                ...
                {                    
                    Global.MainWindow.Dispatcher.InvokeAsync(() =>
                    {
                        ...
                        var songPlayingArtworkcontrol = new SongPlayingArtwork(this.ParentAlbum.baseAlbum, new List<TrackLibrary> { baseTrack}, this.ParentAlbum.baseControlCard, isCarouselVisible: false);
        ...
        private void btnQueNow_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                NormalModeActiveBtnContainer.Visibility = Visibility.Collapsed;
                AdminActiveBtnContainer.Visibility = Visibility.Collapsed;
                AddedToPlaylistInfoButton.Visibility = System.Windows.Visibility.Visible;
                var a = new DoubleAnimation
                {
                    From = 1.0,
                    To = 0.0,
                ...
                using (JukeboxBrainsDBEntities db=new JukeboxBrainsDBEntities())
                {
                    ...
                    foreach (var item in Cards)
                    {
                        Global.mPlayer.dbAddTrackToPlaylist(item.baseTrack, PlaylistId);
                    }
                }
            }
            catch (Exception)
        ...
        private void btnRemove_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                ParentAlbum.PlaylistCount--;
                if (CheckIfAddedInPlaylist(baseTrack))
                {
                    ParentAlbum.PLaylists.Add(new PlaylistWithActionTaken
                    {
                        TrackLibrary = baseTrack,
                ...
                {
                    var first = ParentAlbum.PLaylists.FirstOrDefault(x => x.TrackLibrary == baseTrack);
                    if (first != null)
                    {
                        ParentAlbum.PLaylists.Remove(first);
                    }
        ...
        private void BtnTrackMenu_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (Global.AppControlMode == Global.AppControlModeEnum.Admin)
                {
                    ...
                    {
                        var cardSong = item as LibraryCardSong;
